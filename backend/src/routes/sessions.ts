...
function buildBlockedResponse(session: { id: string; state: string; stage: number }) {
  ...
  return {
    ...
    message: 'Cannot compile. The planning session is ' + r,
    requiredState: 'completed',
  };
}

async function resolveGraph(sess: any): Promise<InternalGraph | null> {
  if (sess.internalGraphId) {
    const prisma = getPrisma();
    const dbGraph = await prisma.internalGraph.findUnique({ where: { id: sess.internalGraphId } });
    if (dbGraph) {
      return {
        id: dbGraph.id,
        metadata: dbGraph.metadata as any,
        nodes: dbGraph.nodes as any,
        edges: dbGraph.edges as any,
      };
    }
  }
  return parseGraphDraft(sess.workflowDraft);
}
...
sessionsRouter.get('/sessions/:id/draft', requireAuth, async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      state: sess.state,
      stage: sess.stage,
      draft: sess.workflowDraft,
      missingFields: sess.missingFields,
      collectedAnswers: sess.collectedAnswers,
    });
  } catch (err) { next(err); }
});
...
sessionsRouter.post('/sessions/:id/compile', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!g) {
      res.status(404).json({ error: 'No workflow draft found', sessionId: id, state: sess.state });
      return;
    }
    ...
    res.json({
      ...
      warnings: r.warnings,
      metadata: {
        workflowName: g.metadata.name || 'Untitled',
        nodeCount: r.workflow.nodes.length,
